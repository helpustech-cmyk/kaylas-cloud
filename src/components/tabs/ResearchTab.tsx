...
export function ResearchTab() {
  ...
  useEffect(() => {
    fetch(API_BASE + '/content?type=research')
      .then(r => r.json())
    ...
    <div style={{ padding: '40px 48px', maxWidth: 1000 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginTop: 24 }}>
        {researchTopics.map(rt => {
          const matchingPosts = posts.filter(p => p.type === 'research' || p.series === rt.tag)
          return (
            <div key={rt.tag} style={{ border: '1px solid oklch(26% 0.005 240)', padding: 16, minHeight: 140, display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ marginTop: 'auto', fontFamily: 'Geist Mono, monospace', fontSize: 11, color: 'oklch(55% 0.005 240)' }}>
                {loading ? 'loading...' : matchingPosts.length + ' post' + (matchingPosts.length !== 1 ? 's' : '')}
              </div>
            </div>
          )
        })}
